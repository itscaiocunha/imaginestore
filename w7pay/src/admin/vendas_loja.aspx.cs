        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                using (IDataReader reader = DatabaseFactory.CreateDatabase("ConnectionString").ExecuteReader(CommandType.Text,
                              "select l.id, max(l.name) as nomeloja, count(*) as qtde, cast(sum(v.[value]) as decimal(10,2)) as valor from vendas v join locais l on l.id = v.location_id group by l.id order by valor desc"))
                {
                    while (reader.Read())
                    {
