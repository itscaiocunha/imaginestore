        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                using (IDataReader reader = DatabaseFactory.CreateDatabase("ConnectionString").ExecuteReader(CommandType.Text,
                              "select p.id, max(p.name) as nomeproduto, count(*) as qtde, cast(sum(v.[value]) as decimal(10,2)) as valor from vendas v join produtos p on p.id = v.good_id where p.manufacturer_id = '" + Session["idempresa"].ToString() + "' group by p.id  order by valor desc"))
                {
                    while (reader.Read())
                    {
